...
private:
    ...
    typename void copy_every_n(typename std::vector<T>::iterator b, typename std::vector<T>::iterator e, typename std::vector<T>::iterator r1, typename std::vector<T>::iterator r2, size_t n);
};
...
std::vector<T> CardShuffler<T>::GenerateDeck(size_t size)
{
    ...
    // copy the deck - is this really needed here for the old code???
    // m_DeckCopy = m_deck;
    return m_deck;
}
...
void CardShuffler<T>::PerformShuffle(ShuffleType shuffleType)
{
    ...
    switch (shuffleType)
    {
        ...
        case ShuffleType::FISHER_YATES:
        {
            ...
            {
                unsigned __int64 r = m_urng() % (i + 1);
                std::swap(m_deck[i], m_deck[r]);
            }
            break;
        }

        // For the inverse outshuffle and inverse inshuffle
        case ShuffleType::EXP_INV_INSHUFFLE:
        case ShuffleType::EXP_INV_OUTSHUFFLE:
        {
            ...
            */
            ...
            if (shuffleType == ShuffleType::EXP_INV_INSHUFFLE)
            {
                if (bIsDeckOdd)
                {
                    // inverse in, n = odd
            ...
            if (shuffleType == ShuffleType::EXP_INV_OUTSHUFFLE)
            {
                if (bIsDeckOdd)
                {
                    // inverse out, n = odd
        ...
        case ShuffleType::EXP_OUTSHUFFLE:
        case ShuffleType::EXP_INSHUFFLE:
        {
            // calculate the two halves of the deck, and assign them accordingly
            ...
            // https://www.techiedelight.com/split-array-two-parts-java/
            size_t half1, half2;
            if (shuffleType == ShuffleType::EXP_INSHUFFLE)
                half1 = (uDeckSize / 2);
            else
                half1 = ((uDeckSize + 1) / 2);
            ...
            for (int nIndex = 0, i = 0; i < minSize; i++, nIndex += 2)
            {
                ...
                if (shuffleType == ShuffleType::EXP_OUTSHUFFLE)
                {
        ...
        case ShuffleType::INSHUFFLE:
        {
            ...
            for (unsigned i = 0; i < uDeckSize; i++)
            {
                if ((shuffleType == ShuffleType::OUTSHUFFLE) || (shuffleType == ShuffleType::INV_OUTSHUFFLE))
                {
                    if (bIsDeckOdd)
                        // O(p) = 2p mod n, n odd
                ...
                if ((shuffleType == ShuffleType::INSHUFFLE) || (shuffleType == ShuffleType::INV_INSHUFFLE))
                {
                    if (bIsDeckOdd)
                ...
                if (nIndex != i)
                {
                    if ((shuffleType == ShuffleType::INV_INSHUFFLE) || (shuffleType == ShuffleType::INV_OUTSHUFFLE))
